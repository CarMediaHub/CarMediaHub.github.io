...
- **draining**: 새 Gateway 요청을 거부하고 기존 요청을 최대 5초 기다립니다. 시간이 초과되어도 미디어 스트림을 강제로 취소하지 않고 업그레이드를 취소해 트래픽을 복원합니다.
...
현재 Core 업그레이드 경로는 새 패키지를 별도 위치에 staging하고 서명, 다이제스트, SDK 범위, Manifest와 패키지 진입점을 검증한 뒤 설치 범위 drain을 수행하고 동일한 설치 ID와 논리 경로의 버전을 갱신합니다. 새 패키지가 `/health`를 선언하면 고정 경로를 검사하고 실패 시 이전 Manifest, 권한과 애플리케이션 메타데이터를 복원합니다. 이전 패키지와 범위 데이터는 롤백을 위해 보존됩니다. 클러스터 전체 drain, 마이그레이션 실패 롤백과 실제 배포 업그레이드 훈련은 여전히 릴리스 게이트입니다. 데이터 마이그레이션은 버전과 범위를 가지며 멱등적이고 Core는 SQL이나 데이터베이스 연결을 플러그인에 제공하지 않습니다.
