...
현재 Core 업그레이드 경로는 새 패키지를 별도 위치에 staging하고 서명, 다이제스트, SDK 범위, Manifest와 패키지 진입점을 검증한 뒤 동일한 설치 ID와 논리 경로의 버전을 갱신합니다. 이전 패키지와 범위 데이터는 롤백을 위해 보존됩니다. 현재 경로는 패키지 무결성과 런타임 호환성을 검증하지만 실제 Worker health probe, drain 조정 및 자동 실패 롤백은 아직 릴리스 게이트입니다. 데이터 마이그레이션은 버전과 범위를 가지며 멱등적이고 Core는 SQL이나 데이터베이스 연결을 플러그인에 제공하지 않습니다.
