...
运行时数据路由强制当前组织、用户和插件安装实例作用域。插件安装、受管组件、服务绑定和应用入口注册等控制面路由仅限管理员使用，并且在 Core v1 中属于部署级资源，不对外宣称多组织隔离。HTTP 成功状态不会授予插件额外能力；Core 会在每次逻辑操作时重新校验已安装 Manifest。
...
测速请求按认证用户限流：Core 同一用户同时只允许一个测速，并且一分钟内最多完成四次测速。超出限制时返回 `429` 和可重试的诊断错误。配额只在用户自己的部署中生效，测速数据不会发送到 CarMediaHub 服务。
...
凭据创建后不再返回明文。`POST /api/credentials/:id/rotate` 会原子撤销有效旧凭据并保存替换值，不返回明文。创建或轮换时可以使用未来的 canonical UTC ISO-8601 `expiresAt` 限定凭据有效期；过期凭据不会出现在列表中，也不会被注入请求。浏览器诊断只返回逻辑 target、会话和任务元数据，绝不返回 Cookie、Profile、CDP、密码、Token、任意 URL 或宿主路径。
