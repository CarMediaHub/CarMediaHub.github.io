...
插件仓库维护一份机器校验的 16 个参考逻辑键清单，覆盖 site-gateway 设计中的集成边界。每个参考键必须且只能对应一条迁移记录，但迁移记录不等于允许分发上游适配器。当前公开集合包含 7 个契约示例：WDR 媒体、AList 与 Mihomo 服务桥、浏览器会话契约，以及中性的适配器示例。需要浏览器身份、导入登录态、媒体提取、远程会话或高风险上游行为的条目保持审查状态，不复制到公开包。清单只包含逻辑键和风险类别，不包含上游域名、Cookie、宿主路径或运行配置。
