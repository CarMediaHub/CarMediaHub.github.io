...
插件包声明身份、三语元数据、SDK 兼容范围、运行时要求、路由、能力和可选服务绑定。清单是授权请求，不是自动获得权限。

插件还可以声明 Core 基础组件依赖（组件 ID、角色和可选标记）。安装或升级时，Core 必须确认组件属于受管目录、角色匹配、安装记录已验证且健康；必需依赖不可用时拒绝准入，可选依赖则由插件提供降级路径。该声明不会把组件可执行路径、命令、进程或宿主环境变量暴露给插件。
...
当前 `browser` 能力已进入 SDK v0 Wire 契约，提供不透明、受作用域限制的会话和有界 Core-owned 任务队列：`browser.session.request`、`browser.session.list`、`browser.session.revoke`、`browser.task.enqueue`、`browser.task.list` 和 `browser.task.cancel`。Core 已按组织、用户和插件安装实例实现会话/任务的作用域持久化、列表、撤销和取消。受管 Browser Worker driver 已通过真实静音 Chrome smoke，验证允许 Origin、未知 Origin 阻断和作用域 User Data 隔离；脚本执行、Cookie 导入、媒体提取、签名浏览器分发、重定向/WebSocket/Worker 覆盖及跨平台兼容矩阵仍未完成。该契约不会暴露浏览器 Profile、Cookie、CDP 地址、宿主路径、进程或任意 URL/脚本。
...
`db` 授予的是受限的逻辑数据空间，不是数据库连接。每次读写都由 Core 绑定到当前组织、用户和插件安装实例。插件不能选择其他用户、指定 schema、获得 DSN，也不能执行任意 SQL。无论 Core 使用 SQLite、PostgreSQL、MySQL 或运营者管理的其他兼容数据库，SDK 语义保持一致。

数据列表具有确定性：记录按 key 顺序返回，`prefix` 按字面量 key 前缀匹配，而不是 SQL pattern。这样可以保证不同数据库后端的行为一致。
...
停用、卸载和删除数据仍是三个独立的生命周期操作。卸载前必须先停用；卸载会移除活动入口和运行时授权，但会保留该安装实例的作用域数据，直到用户明确删除。之后重新安装相同版本的插件会获得新的安装实例身份。

## 适配器发布

插件目录区分原生媒体应用、本地服务桥接、上游适配器、浏览器桥接和社区包。目录条目不等于运行代码的批准。需要浏览器身份、外部网络、媒体提取或高风险上游的包，必须使用隔离运行时，经过明确的能力审查，并在公开分发前具备专门的泄露与回滚测试。

公开的 `browser-session-contract-example` 用于验证这一边界。它已经通过真实隔离 Worker、Core Broker 和 Gateway 链路测试不透明会话与受限任务元数据，但不会启动真实浏览器或连接上游网站；它不是浏览器自动化或登录态集成。新增的 `uyous-media-contract` 将同一边界扩展到受限媒体提取任务契约；它不访问 Profile、Cookie、CDP、yt-dlp、FFmpeg 或真实视频平台。
...
Core 在当前组织、用户和插件安装实例作用域内，为每个 binding 限制最多 10 个活动请求，并拒绝超过 1 MiB 的响应正文。同名 binding 在其他作用域拥有独立配额。请求成功、失败、超时或取消时都会释放配额。

仓库中的 `service-binding-adapter-example` 是中性的参考实现，只验证契约，不针对具体网站，也不提供公共代理。

插件数据迁移可以使用 SDK `database().migrateBatch()` 一次提交多个版本。Core 会在当前用户和插件安装实例作用域内使用数据库事务；任一版本冲突或参数无效时，整批迁移回滚，不留下部分迁移台账。单条 `migrate()` 仍保留用于简单、幂等的初始化。

## 迁移策略

插件仓库维护一份机器校验的 16 个参考逻辑键清单，覆盖 site-gateway 设计中的集成边界。每个参考键必须且只能对应一条迁移记录，但迁移记录不等于允许分发上游适配器。当前公开集合包含 10 个示例包，其中包括 Core-owned 的浏览记录能力、WDR 媒体、AList/rclone/Mihomo 服务桥、浏览器/媒体契约，以及中性的适配器示例。需要浏览器身份、导入登录态、媒体提取、远程会话或高风险上游行为的条目保持审查状态，不复制到公开包。清单只包含逻辑键和风险类别，不包含上游域名、Cookie、宿主路径或运行配置。
