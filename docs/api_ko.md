...
관리자는 관리 화면을 사용합니다. API는 실행 전 크기, 메서드, capability와 리소스 경계를 검증합니다. 주요 그룹은 `/api/users`, `/api/components`, `/api/media-roots`, `/api/media-sources`, `/api/plugins`, `/api/service-bindings`, `/api/credentials`, `/api/jobs`, `/api/browser/sessions` 및 `/api/browser/tasks`입니다.
