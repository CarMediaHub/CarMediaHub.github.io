...
인증된 사용자는 `POST /api/auth/password`에 `currentPassword`와 12자 이상인 새 비밀번호를 보내 비밀번호를 변경할 수 있습니다. Core는 현재 세션을 유지하고 같은 사용자의 다른 세션을 철회합니다.

런타임 데이터 경로는 현재 조직·사용자·플러그인 설치 인스턴스 범위를 강제합니다. 플러그인 설치, 관리 구성 요소, 서비스 바인딩과 앱 항목 등록 같은 제어면 경로는 관리자 전용 배포 리소스이며, Core v1은 이를 다중 조직 리소스로 제공한다고 약속하지 않습니다. HTTP 성공이 capability를 추가로 부여하지 않으며 Core가 모든 논리 작업에서 설치된 Manifest를 다시 확인합니다.
...
Entry Key는 `POST /api/keys`로 생성되며 Core는 해시만 저장하고 현재 관리자에게 귀속합니다. `GET /api/keys`로 자신의 Key를 조회하고 `POST /api/keys/:id/revoke`로 철회합니다. `POST /api/keys/:id/rotate`는 현재 사용자가 소유한 활성 Key를 원자적으로 철회하고 교체 Key의 평문을 한 번만 반환합니다. `expiresAt`을 지정할 때는 미래의 정규 ISO-8601 UTC 시간이어야 합니다. 다른 사용자의 Key, 반복 또는 이미 철회된 작업은 `404`로 숨겨지며, Key는 소유자의 기존 권한을 확장하지 않습니다.
