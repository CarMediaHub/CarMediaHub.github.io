...
To prepare a release, run Core's explicit `component:prepare-release` command with a data directory, artifact path, component ID, version, platform and artifact ID. The command copies a regular file into `data/staging`, computes its SHA-256 digest and writes an unsigned release record when requested. It never reads a private key, signs a release, overwrites an existing staging artifact or installs the component. An operator-owned signing process must sign the record before the management API accepts it.
