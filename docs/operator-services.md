...
For an operator-owned release workstation, Core also provides an explicit `component:sign-release` command. It reads a selected unsigned record and an Ed25519 private-key file, requires the derived public-key fingerprint to match `release.keyId`, and writes a new signed record without copying the private key into staging or a runtime bundle. Sign one record for each platform, then use `component:assemble-matrix` with an explicit platform list to produce a release matrix. The matrix command rejects a missing platform, duplicate platform, mixed component or version, incomplete provenance, and output overwrite.

The matrix is a release-management artifact, not an installation request. `/api/components/install` accepts only one trusted signed record for the deployment's current platform; submitting a matrix is rejected. The public schemas are available at [`/schemas/component-release.schema.json`](/schemas/component-release.schema.json) and [`/schemas/component-release-matrix.schema.json`](/schemas/component-release-matrix.schema.json). This process provides auditable release records, but does not mean CarMediaHub currently ships official FFmpeg, AList, rclone, Mihomo, or Chromium binaries.
...
AList and rclone WebDAV can be registered as read-only media sources, separately from the AList Web management bridge. Core uses an opaque source handle for scoped listing, probing, playback sessions, and Range reads. Plugins do not receive a WebDAV URL, endpoint, host path, or credential; Core resolves and injects WebDAV credentials at the final request boundary.

Core now has registration, persistence, revocation, health checks, and a read-only WebDAV provider for remote media sources, with local fixtures covering the request boundary. Health checks return only status and a classified diagnostic. WDR can consume a registered source through the SDK, while `alist-web-bridge` remains a bounded management/catalog adapter. The real AList/rclone deployment matrix and WDR production verification are still pending. WebDAV write, delete, upload, arbitrary proxying, and automatic discovery are outside the initial scope.
