...
The `browser` capability currently exposes opaque, scoped sessions and a bounded Core-owned task queue: `browser.session.request`, `browser.session.list`, `browser.session.revoke`, `browser.task.enqueue`, `browser.task.list`, and `browser.task.cancel`. Tasks use registered logical kinds and limited input, and are bound to an active session, organization, user, and plugin installation. Core currently stores and cancels tasks but does not execute navigation, scripts, cookie import, or media extraction. The contract never exposes browser profiles, cookies, CDP endpoints, host paths, processes, or arbitrary URLs. A real Browser Worker and compatibility matrix remain future work.
