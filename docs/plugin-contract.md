...
The `browser` capability currently exposes only opaque, scoped session metadata: `browser.session.request`, `browser.session.list`, and `browser.session.revoke`. A session contains an opaque ID, logical name, purpose, status, and expiry. Core binds it to the organization, user, and plugin installation and revokes it when the user, installation, or capability is revoked. The contract never exposes browser profiles, cookies, CDP endpoints, host paths, processes, arbitrary URLs, or scripts. Browser task execution and real browser compatibility remain future work.
