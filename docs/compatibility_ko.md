...
| 영역 | 현재 상태 | 증거와 경계 |
| --- | --- | --- |
| Node.js Core 런타임 | 검증됨 | 지원 개발 호스트에서 TypeScript 빌드와 Core `153/153` 회귀. |
| SDK와 Wire Protocol v0.1 | 검증됨 | SDK `40/40`, Manifest/오류/Wire 계약과 Memory Runtime 테스트. |
| 공식 플러그인 패키지 | 검증됨 | 6개 패키지 Manifest, 카탈로그 정렬과 패키지 검증 통과. |
