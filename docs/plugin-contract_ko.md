...
패키지는 관리 컴포넌트 ID, 필요한 역할 및 선택 여부로 Core 컴포넌트 의존성을 선언할 수도 있습니다. 설치 또는 업그레이드 전에 Core는 카탈로그 등록, 역할 호환성, 검증된 설치 기록 및 상태를 확인합니다. 필수 의존성을 사용할 수 없으면 설치가 거부되며, 선택 의존성에는 문서화된 축소 경로가 필요합니다. 이 선언은 플러그인에 실행 파일 경로, 명령, 프로세스 또는 호스트 환경 변수를 제공하지 않습니다.
...
`ui.entry`를 선언한 서명 패키지의 경우 Core는 패키지 digest를 다시 계산해 검증한 뒤 인증된 애플리케이션 루트와 명시적 네임스페이스의 `/ui/*` 자산을 제공할 수 있습니다. UI 파일은 검증된 패키지에서만 읽고 Worker가 제공하지 않으며, Core는 허용된 콘텐츠 유형만 사용하고 `no-store` 및 `nosniff`를 반환합니다. UI와 API는 동일한 사용자 세션 및 애플리케이션 경로를 사용하며 패키지 경로나 별도 수신 포트를 받지 않습니다.
...
`db`는 데이터베이스 연결이 아니라 제한된 논리 데이터 저장소를 부여합니다. Core는 각 읽기와 쓰기를 현재 조직, 사용자 및 플러그인 설치 인스턴스에 바인딩합니다. 플러그인은 다른 사용자를 선택하거나 schema를 지정하거나 DSN을 받거나 임의 SQL을 실행할 수 없습니다. Core가 SQLite, PostgreSQL, MySQL 또는 운영자가 관리하는 다른 호환 데이터베이스를 사용해도 SDK 의미는 동일합니다.
...
공개된 `browser-session-contract-example`은 격리 Worker, Core Broker 및 Gateway 경로를 통해 불투명 세션과 제한된 작업 메타데이터를 검증했지만 실제 브라우저를 시작하거나 상위 웹사이트에 연결하지 않으며 브라우저 자동화나 로그인 상태 통합이 아닙니다. 새 `uyous-media-contract`는 같은 경계를 제한된 미디어 추출 작업 계약으로 확장하며 Profile, Cookie, CDP, yt-dlp, FFmpeg 또는 실제 동영상 플랫폼에 접근하지 않습니다.
...
Core는 현재 조직, 사용자 및 플러그인 설치 인스턴스 범위의 각 binding을 활성 요청 최대 10개로 제한하고 1 MiB를 초과하는 응답 본문을 거부합니다. 다른 범위에서 같은 이름의 binding을 사용하면 독립적인 할당량을 가집니다. 성공, 실패, 시간 초과 또는 취소 시 할당량은 해제됩니다.

저장소의 `service-binding-adapter-example`은 중립적인 참조 구현입니다. 특정 웹사이트를 대상으로 하지 않으며 공개 프록시를 제공하지 않습니다.

플러그인 데이터 마이그레이션은 SDK `database().migrateBatch()`로 여러 버전을 원자적으로 제출할 수 있습니다. Core는 현재 사용자와 플러그인 설치 범위에서 트랜잭션을 사용하며, 버전 충돌이나 잘못된 입력이 있으면 전체 배치를 롤백해 부분적인 마이그레이션 기록을 남기지 않습니다. 단일 `migrate()`는 간단한 멱등 초기화용으로 유지됩니다.

## 마이그레이션 정책

플러그인 저장소는 site-gateway 설계의 통합 경계를 나타내는 16개의 논리 참조 키 목록을 기계적으로 검증합니다. 각 참조 키는 정확히 하나의 마이그레이션 기록을 가져야 하지만, 기록이 상류 어댑터의 배포 승인을 의미하지는 않습니다. 현재 공개 컬렉션에는 Core 소유 방문 기록 기능, WDR 미디어, AList/rclone/Mihomo 서비스 브리지, 브라우저/미디어 계약과 중립 어댑터 예제를 포함한 10개의 예제 패키지가 있습니다. 브라우저 ID, 가져온 로그인 상태, 미디어 추출, 원격 세션 또는 고위험 상류 동작이 필요한 항목은 검토 전용으로 유지되며 공개 패키지에 복사하지 않습니다. 목록에는 논리 키와 위험 분류만 포함하며 상류 도메인, Cookie, 호스트 경로 또는 실행 구성을 포함하지 않습니다.
