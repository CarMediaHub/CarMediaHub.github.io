...
플러그인 저장소는 site-gateway 설계의 통합 경계를 나타내는 16개의 논리 참조 키 목록을 기계적으로 검증합니다. 각 참조 키는 정확히 하나의 마이그레이션 기록을 가져야 하지만, 기록이 상류 어댑터의 배포 승인을 의미하지는 않습니다. 현재 공개 컬렉션에는 WDR 미디어, AList 및 Mihomo 서비스 브리지, 브라우저/미디어 계약과 중립 어댑터 예제를 포함한 8개의 계약 예제가 있습니다. 브라우저 ID, 가져온 로그인 상태, 미디어 추출, 원격 세션 또는 고위험 상류 동작이 필요한 항목은 검토 전용으로 유지되며 공개 패키지에 복사하지 않습니다. 목록에는 논리 키와 위험 분류만 포함하며 상류 도메인, Cookie, 호스트 경로 또는 실행 구성을 포함하지 않습니다.
