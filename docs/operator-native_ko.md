...
Native 설치 관리자, 서비스 계정 및 ACL 설정, Linux 시스템 서비스 생성, 깨끗한 시스템 설치, 컴포넌트 배포, 업데이트, 롤백과 플랫폼 간 복구는 별도의 출시 기준입니다. 이 기준이 완료되기 전에는 데이터베이스, Worker, 플러그인 또는 디버그 포트를 인터넷에 공개하지 마십시오.
