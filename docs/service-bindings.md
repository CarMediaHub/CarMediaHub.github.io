...
An operator creates a binding for an installed plugin instance. Core stores the private endpoint and credentials, checks the declared binding name, and applies origin, method, redirect, body-size, response-size, scope-isolated concurrency, timeout, and cancellation limits. The initial concurrency limit is ten active requests per organization, user, installation, and binding scope.
