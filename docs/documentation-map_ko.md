...
| 독자 | 시작 문서 | 주요 질문 |
|---|---|---|
| 일반 사용자 | [프로젝트 유래](project-origin_ko.md)와 사용자 안내 | CarMediaHub는 무엇이며 무엇을 준비해야 하나요? |
| 운영자 | 배포 및 운영 안내 | 설치, 보안 강화, 백업, 업데이트와 진단은 어떻게 하나요? |
| 플러그인 개발자 | [플러그인 계약](plugin-contract_ko.md)과 SDK 참고 | 플러그인을 개발, 격리, 테스트, 배포하고 유지하려면 어떻게 하나요? |
...
운영자
  요구 사항 -> Docker -> Native -> 네트워크 진입점 -> 서비스와 바인딩
  -> 인증 -> 백업과 복구 -> 업데이트 -> 진단

개발자
  SDK 시작하기 -> Manifest -> 수명 주기 -> 컨텍스트와 국제화 -> capability
  -> 데이터와 작업 -> UI -> 프록시 어댑터 -> 네이티브 플러그인
  -> 테스트 -> 패키징, 서명과 보안 보고서

참고
  API -> 오류 -> 이벤트 -> 구성 -> 호환성 매트릭스
  -> 알려진 제한 -> 버전과 마이그레이션
```
