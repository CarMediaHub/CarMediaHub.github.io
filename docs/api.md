...
Credential creation never returns plaintext again. `POST /api/credentials/:id/rotate` atomically revokes an active credential and stores a replacement value without returning plaintext. An optional future canonical UTC ISO-8601 `expiresAt` can bound a credential; expired credentials are omitted from listings and cannot be injected into requests. Browser diagnostics return logical target/session/task metadata only; they never return Cookie, Profile, CDP, password, token, arbitrary URL or host path data.
...
Entry keys use `POST /api/keys` to create a hashed, user-owned opaque entry URL, `GET /api/keys` to list the current administrator's keys, and `POST /api/keys/:id/revoke` to revoke one. `POST /api/keys/:id/rotate` atomically revokes an active owned key and returns its replacement plaintext once. `expiresAt`, when supplied, must be a future canonical ISO-8601 UTC timestamp. Cross-user, repeated, and already-revoked operations are hidden as `404`; a key does not expand the permissions of its owner.
