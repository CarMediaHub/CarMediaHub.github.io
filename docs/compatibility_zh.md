...
| 范围 | 当前状态 | 证据与边界 |
| --- | --- | --- |
| Node.js Core 运行时 | 已验证 | 支持开发主机上的 TypeScript 构建和 Core `153/153` 回归。 |
| SDK 与 Wire Protocol v0.1 | 已验证 | SDK `40/40`，Manifest/错误/Wire 契约和 Memory Runtime 测试。 |
| 官方插件包 | 已验证 | 6 个包的 Manifest、目录一致性和分发包校验通过。 |
...
| Chromium/浏览器桥接 | 契约/夹具 | 目标 allowlist、静音受管驱动、任务/结果边界和 Worker 夹具通过；Chromium 签名分发、真实导航和跨平台矩阵待完成。 |
