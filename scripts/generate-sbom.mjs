...
const windowsCmdPath = () => path.join(path.parse(process.execPath).root, "Windows", "System32", "cmd.exe");
const list = () => process.platform === "win32" ? execFileSync(windowsCmdPath(), ["/d", "/s", "/c", "pnpm list --json --prod --depth Infinity"], { cwd: root, encoding: "utf8" }) : execFileSync("pnpm", ["list", "--json", "--prod", "--depth", "Infinity"], { cwd: root, encoding: "utf8" });
